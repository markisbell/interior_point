...
% 
% ToDo:
%      - Debug Informationen einfuegen, je nachdem wie hoch die Verbositaet ist
%       - Chek if the problem is infeasible from the beginning or during
%       solution
%      - Visualisierung des Optimierungsproblems
%
% Markus Bell 30.12.2021
%
...
function ip_result = IP_qpqc_n(optimmodel,options)
    ...
    if LinInequFeas > 0 || QuadInequFeas > 0
      ...
      fprintf(' \n');
      fprintf('INFO: x0 is a feasible starting vector\n', QuadInequFeas);
      fprintf(' \n');
    end
    %----------------------------------------------------------------------
    ...
    %----------------------------------------------------------------------
    %Initialize the single variables taht are not yet in the options
    %----------------------------------------------------------------------
    %Duality measure
    rho = 1;
    ...
    %Security factor (to guarantee stricly positiveness of slack and lagrange multiplier)
    gamma = 0.995;
    ...
    %Don't stop with the iterative process till the we have a certain value
    ...
    %Try to solve this until certain criteria are fulfilled
     while ((norm(rE,2) >= options.eps_prim) || (norm(rS,2) >= options.eps_dual) || (-g'*muk >= options.eps_gap)) && (iter <= options.iter_max)
        ...
        %==================================================================
        %Solution of the KKT through Newton
        %==================================================================
        ...
        %Build up the perturbed KKT equations with slack, etc.
        ...
        %------------------------------------------------------------------
        %Line Search and determination of the step lenght
        %------------------------------------------------------------------
